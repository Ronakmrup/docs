...
<CardGroup cols={2}>
  <Card title="Input Audio Quality" icon="microphone">
    **Do:**

    - Use 16 kHz sample rate when possible
    - Capture raw microphone input
    - Maintain consistent gain levels

    **Don't:**

    - Apply noise reduction before Sanas
    - Use heavily compressed audio
    - Mix sample rates mid-stream
  </Card>
  <Card title="ASR Integration" icon="plug">
    **Do:**

    - Send Sanas output directly to ASR
    - Match ASR expected sample rate
    - Test with clean audio first

    **Don't:**

    - Apply additional processing
    - Convert formats multiple times
    - Cache/buffer excessively
  </Card>
  <Card title="Error Handling" icon="shield">
    **Do:**

    - Monitor processor state changes
  ...
  <Card title="Production Deployment" icon="server">
    ...
    **Don't:**
    ...
    - Deploy without testing peak load
    - Ignore p95/p99 latency
...
<AccordionGroup>
  <Accordion title="High WER despite using AI Voice Agent" icon="triangle-exclamation">
    **Possible causes:**

    1. Sample rate mismatch between Sanas output and ASR input
    2. Audio format conversion errors
    3. ASR model not suitable for domain

    **Debug steps:**

    - Save and listen to Sanas output to confirm noise is removed
    - Test ASR directly on clean audio to verify ASR configuration
    - Check sample rate consistency throughout pipeline
  </Accordion>
  <Accordion title="Audio artifacts or distortion" icon="triangle-exclamation">
    **Symptoms:** Robotic voice, clipping, echo

    **Common causes:**

    1. Input audio is already heavily processed
    2. Sample rate conversion issues
    3. Buffer size mismatches

    **Solutions:**

    - Use raw microphone input (before any processing)
    - Ensure consistent sample rate (16kHz recommended)
    - Use correct chunk sizes (320 samples for 16kHz)
  </Accordion>
  <Accordion title="Processor creation fails with AI Voice Agent" icon="triangle-exclamation">
    **Common issues:**

    - Model name typo (must be exactly `ASR_NC3.0_VAD0.6`)
    - Sample rate \> 16000 (not supported by this model)
    - Network connectivity to Sanas Cloud
    ...
    **Verify:**
    ...
    # Should show: ASR_NC3.0_VAD0.6 and ≤16000
    ```
  </Accordion>
</AccordionGroup>
...
<Note>
  **Need help?** Contact our support team at [support@sanas.ai](mailto:support@sanas.ai) or [raise a ticket](/support).
</Note>